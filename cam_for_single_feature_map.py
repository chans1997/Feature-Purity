...
def show_cam_on_image(img, mask):
    ...
    cam = cam / np.max(cam)
    cv2.imwrite("resnet50/test/both.jpg", np.uint8(255 * cam))

def show_cam_on_image_single_map(img, mask,i):
    heatmap = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)
    heatmap = np.float32(heatmap) / 255
    cam = heatmap + np.float32(img)
    cam = cam / np.max(cam)
...
class GradCAM(object):
    ...
    def __call__(self, inputs, index):
        ...
        cam_total = np.sum(cam, axis=0)  # [H,W]
        cam = np.maximum(cam, 0)  # ReLU
        cam_total = np.maximum(cam_total, 0)
        # 数值归一化
        # cam -= np.min(cam)
        # cam /= np.max(cam)
        # #resize to 224*224
        # cam = cv2.resize(cam, (224, 224))
        cam_total -= np.min(cam_total)
        if np.max(cam_total) != 0:
            cam_total /= np.max(cam_total)
...
if __name__ == '__main__':
    ...
    for i in range(mask.shape[0]):
        single_mask = mask[i, :, :]
        single_mask -= np.min(single_mask)
        if np.max(single_mask) != 0:
            single_mask /= np.max(single_mask)
        single_mask = cv2.resize(single_mask, (224, 224))
        show_cam_on_image_single_map(img, single_mask,i)
    show_cam_on_image(img, mask_total)
    # gb_model = GuidedBackpropReLUModel(model=model, use_cuda=True)
    # print(model._modules.items())
    # gb = gb_model(input, index=target_index)
    # gb = gb.transpose((1, 2, 0))
    # cam_mask = cv2.merge([mask_total, mask_total, mask_total])
    # cam_gb = deprocess_image(cam_mask * gb)
    # gb = deprocess_image(gb)
    #
    # cv2.imwrite('resnet_dog_gb1_1.jpg', gb)
    # cv2.imwrite('resnet_dog_cam_gb1_1.jpg', cam_gb)
